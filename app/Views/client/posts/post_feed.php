...
<div class="page-container">
    <header class="page-header">
        ...
        <a href="<?= site_url('client/komunitas/create') ?>" class="btn btn-primary"><i class="ri-add-line"></i> Buat Postingan Baru</a>
    </header>
            ...
            <?php if (!empty($posts)) : ?>
                <?php foreach ($posts as $post) : ?>
                    <div class="post-card" data-topic="<?= esc($post['topic']) ?>" data-user-id="<?= esc($post['user_id']) ?>">
                        ...
                        <h3 class="post-title"><a href="<?= site_url('client/komunitas/view/' . $post['id']) ?>"><?= esc($post['title']) ?></a></h3>
...
<script src="<?= base_url('assets/js/pages/client_community.js') ?>"></script>
<?= $this->endSection() ?>