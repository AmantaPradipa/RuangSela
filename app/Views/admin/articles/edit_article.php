...
<div class="editor-container">
    ...
    <header class="editor-header">
        ...
        <button type="submit" form="article-form" class="btn-submit">Perbarui & Publikasikan</button>
    </header>
    ...
    <div class="editor-card">
        <form id="article-form" action="<?= site_url('admin/articles/update/' . $article['id']) ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <input type="hidden" name="_method" value="PUT">
            <input type="text" name="title" class="title-input" placeholder="Judul Artikel Anda" value="<?= old('title', $article['title']) ?>">

            <div class="form-group">
                <label for="author_id" class="field-label">Penulis</label>
                <select name="author_id" id="author_id" class="input-field" required>
                    <option value="">Pilih Penulis</option>
                    <?php foreach ($authors as $author) : ?>
                        <option value="<?= esc($author['id']) ?>" <?= old('author_id', $article['author_id']) == $author['id'] ? 'selected' : '' ?>><?= esc($author['first_name'] . ' ' . $author['last_name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="excerpt" id="excerpt" class="textarea-field" placeholder="Ringkasan singkat artikel Anda..."><?= old('excerpt', $article['excerpt']) ?></textarea>
            </div>

            <div class="form-group">
                <label for="featured_image" class="field-label">Gambar Unggulan (Opsional)</label>
                <?php if (!empty($article['featured_image'])) : ?>
                    <p>Gambar saat ini: <img src="<?= asset_url($article['featured_image']) ?>" width="100"></p>
                <?php endif; ?>
            ...
            <div class="form-group">
                ...
                <textarea name="content" id="content" class="main-textarea" placeholder="Tuliskan isi artikel Anda di sini..."><?= old('content', $article['content']) ?></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="is_published" id="is_published" class="input-field">
                    ...
                    <option value="1" <?= old('is_published', $article['is_published']) == '1' ? 'selected' : '' ?>>Publikasikan</option>
                </select>
            </div>

            <div class="tags-section">
                <label class="field-label">Tags (Pisahkan dengan koma)</label>
                <input type="text" name="tags" class="input-field" placeholder="Contoh: kecemasan, stres, mindfulness" value="<?= old('tags', '' ) ?>"> <!-- Tags not stored in DB yet -->
            </div>
